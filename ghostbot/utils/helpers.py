"""Utility functions for ghostbot."""
...
def build_status_content(
    *,
    ...
    lines = [
        f"\ud83d\udc08 ghostbot v{version}",
        f"\ud83e\udde0 Model: {model}",
        token_line,
        f"\ud83d\udcda Context: {ctx_used_str}/{ctx_total_str} ({ctx_pct}%)",
        f"\ud83d\udcac Project history: {session_msg_count} messages",
        f"\u23f1 Uptime: {uptime}",
    ]
    if mode:
        lines.append(f"\ud83d\udee0 Mode: {mode}")
    if active_project or active_project_path:
        project_line = f"\ud83d\udcc1 Active Project: {active_project or 'unknown'}"
        if active_project_path:
            project_line += f" ({active_project_path})"
        lines.append(project_line)
    if branch or repo_root:
        repo_line = f"\ud83c\udf3f Repo: {branch or 'unknown'}"
        if repo_root:
            repo_line += f" ({repo_root})"
        lines.append(repo_line)
    if approval_mode:
        lines.append(f"\ud83d\udd10 Approval: {approval_mode}")
    if isolation:
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("ghostbot") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
