"""WebSocket server channel: ghostbot acts as a WebSocket server and serves connected clients."""
...
from ghostbot.config.schema import Base
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      ghostbot and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-ghostbot-Auth: <secret>``.
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-ghostbot-Auth") or headers.get("x-ghostbot-auth")
    if not header_token:
        return False
