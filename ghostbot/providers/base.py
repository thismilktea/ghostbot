...
from ghostbot.utils.helpers import image_placeholder_text
...
class LLMProvider(ABC):
    ...
    _MAX_RETRY_AFTER_DELAY = 10
    _PERSISTENT_MAX_DELAY = 10
    _PERSISTENT_IDENTICAL_ERROR_LIMIT = 10
    _RETRY_HEARTBEAT_CHUNK = 10
    ...
    async def chat_stream(
        ...
        if on_content_delta and (response.content or response.reasoning_content):
            try:
                await on_content_delta(response.content or "", reasoning_delta=response.reasoning_content)
            except TypeError:
                if response.content:
    ...
    async def _run_with_retry(
        ...
        *,
        ...
        while True:
            ...
            delay = base_delay if retry_after is None else min(retry_after, self._MAX_RETRY_AFTER_DELAY)
            if persistent:
