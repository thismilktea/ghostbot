...
from ghostbot.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from ghostbot.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/your-org/ghostbot",
    "X-OpenRouter-Title": "ghostbot",
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply ghostbot attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _handle_error(e: Exception) -> LLMResponse:
        ...
        # ==========================================
        # 🔥 新增这一行：强行打印底层的真实异常和报文！
        # ==========================================
        print(f"\n🚨 [底层 API 报错揭秘] 异常类型: {type(e).__name__}\n详细报文: {msg}\n")
        ...
        try:
            ...
            while True:
                ...
                if on_content_delta and chunk.choices:
                    ...
                    if reasoning:
                        if not is_thinking:
                            # 初始进入思考模式，发送一个明显的开始标记
                            await on_content_delta("\n\n> **🤔 思维链分析中...**\n> ")
                            is_thinking = True

                        # 核心修复：将换行符替换为 Markdown 引用符，防止格式崩坏
                        processed_reasoning = reasoning.replace("\n", "\n> ")
                        await on_content_delta(processed_reasoning)

                    if text:
                        if is_thinking:
                            # 思考转正式回答的分割线
                            await on_content_delta("\n\n---\n\n")
                            is_thinking = False  # 状态重置
