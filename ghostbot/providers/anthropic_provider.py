...
from ghostbot.providers.base import LLMProvider, LLMResponse, ToolCallRequest
...
class AnthropicProvider(LLMProvider):
    ...
    def __init__(
        ...
        self.header_profile = header_profile or os.environ.get("GHOSTBOT_ANTHROPIC_HEADER_PROFILE")
        ...
        if self.header_profile == "claude_code" or os.environ.get("GHOSTBOT_DEBUG_ANTHROPIC"):
            import httpx

            hooks: dict[str, list[Callable[..., Awaitable[None]]]] = {"request": [], "response": []}
            if self.header_profile == "claude_code":
                hooks["request"].append(self._apply_claude_code_headers)
            if os.environ.get("GHOSTBOT_DEBUG_ANTHROPIC"):
                hooks["request"].append(self._debug_request)
                hooks["response"].append(self._debug_response)
            client_kw["http_client"] = httpx.AsyncClient(event_hooks=hooks)
    ...
    @classmethod
    def _redact_headers(cls, headers: Any) -> dict[str, str]:
        redacted: dict[str, str] = {}
        for key, value in dict(headers).items():
            lowered = key.lower()
            if lowered in {"authorization", "x-api-key", "api-key", "cookie"}:
    ...
    @staticmethod
    def _json_preview(data: bytes, limit: int = 4000) -> str:
    ...
    async def _apply_claude_code_headers(self, request: Any) -> None:
        request.headers["user-agent"] = "claude-cli/2.1.148"
        for key in list(request.headers):
            lowered = key.lower()
            if lowered.startswith("x-stainless"):
                del request.headers[key]

    async def _debug_request(self, request: Any) -> None:
        print("\n[GHOSTBOT_DEBUG_ANTHROPIC] request")
        print(f"{request.method} {request.url}")
        print(json.dumps(self._redact_headers(request.headers), ensure_ascii=False, indent=2))
        print(self._json_preview(request.content))

    async def _debug_response(self, response: Any) -> None:
        await response.aread()
        print("\n[GHOSTBOT_DEBUG_ANTHROPIC] response")
        print(f"status={response.status_code} url={response.url}")
        print(json.dumps(self._redact_headers(response.headers), ensure_ascii=False, indent=2))
        print(self._json_preview(response.content))
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("ghostbot_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
