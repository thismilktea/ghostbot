...
from ghostbot.agent.tools.filesystem import ListDirTool, _FsTool
...
class _SearchTool(_FsTool):
    ...
    def __init__(
        ...
        priority_bucket_provider: Callable[[], dict[str, float]] | None = None,
    ):
        super().__init__(workspace, allowed_dir, extra_allowed_dirs)
        self._priority_bucket_provider = priority_bucket_provider

    @staticmethod
    def _normalize_bucket_path(path: str) -> str:
        return path.replace("\\", "/").strip("/")

    def _get_priority_bucket(self) -> dict[str, float]:
        if not self._priority_bucket_provider:
            return {}
        try:
            bucket = self._priority_bucket_provider() or {}
        except Exception:
            return {}
        return {self._normalize_bucket_path(path): score for path, score in bucket.items()}

    def _bucket_score(self, display_path: str, bucket: dict[str, float]) -> float:
        normalized = self._normalize_bucket_path(display_path.rstrip("/"))
        return bucket.get(normalized, 0.0)
...
class GlobTool(_SearchTool):
    ...
    def description(self) -> str:
        return (
            ...
            "Results are prioritized by the current session bucket, then by modification time. "
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            bucket = self._get_priority_bucket()
            ...
            for entry in self._iter_entries(
                ...
                if _match_glob(rel_path, entry.name, pattern):
                    ...
                    try:
                        ...
                        mtime = 0.0
                    bucket_score = self._bucket_score(display, bucket)
                    matches.append((display, bucket_score, mtime))

            if not matches:
                return f"No paths matched pattern '{pattern}' in {path}"

            matches.sort(key=lambda item: (-item[1], -item[2], item[0]))
            ordered = [name for name, _, _ in matches]
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            matching_files: dict[str, tuple[float, float]] = {}
            counts: dict[str, int] = {}
            ...
            for file_path in self._iter_files(target):
                ...
                bucket_score = self._bucket_score(display_path, bucket)
                ...
                for idx, line in enumerate(lines, start=1):
                    ...
                    content_blocks.append((display_path, block))

                if file_had_match:
                    matching_files[display_path] = (bucket_score, mtime)
            ...
            if output_mode == "files_with_matches":
                ...
                if not content_blocks:
                    ...
                    ordered_blocks = sorted(
                        content_blocks,
                        key=lambda item: (
                            -matching_files.get(item[0], (0.0, 0.0))[0],
                            -matching_files.get(item[0], (0.0, 0.0))[1],
                            item[0],
                        ),
                    )
                    paged, truncated = _paginate(ordered_blocks, limit, offset)
                    rendered_blocks: list[str] = []
                    result_chars = 0
                    size_truncated = False
                    for _, block in paged:
                        extra_sep = 2 if rendered_blocks else 0
