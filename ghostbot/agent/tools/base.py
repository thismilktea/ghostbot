...
class Schema(ABC):
    ...
    Concrete types live in :mod:`ghostbot.agent.tools.schema`; all implement
...
class Tool(ABC):
    ...
    def side_effect_level(self) -> str:
        """Policy-facing side effect category for this tool."""
        return "none" if self.read_only else "unknown"

    @property
    def risk_tags(self) -> frozenset[str]:
        """Policy-facing risk tags for this tool."""
        return frozenset()
