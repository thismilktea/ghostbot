...
from ghostbot.agent.tools.base import ToolResult, tool_parameters
from ghostbot.agent.tools.schema import IntegerSchema, StringSchema, tool_parameters_schema
from ghostbot.agent.tools.filesystem import _FsTool
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not fp.exists():
                ...
                message = f"Successfully created {fp} with 1 cell"
                summary = build_change_summary(fp, "", after)
                if summary:
            ...
            if edit_mode == "delete":
                ...
                after = json.dumps(nb, indent=1, ensure_ascii=False)
                fp.write_text(after, encoding="utf-8")
                message = f"Successfully deleted cell {cell_index} from {fp}"
                summary = build_change_summary(fp, before, after)
                if summary:
                    return ToolResult(message, {"change_summary": summary})
                return message
            ...
            if edit_mode == "insert":
                ...
                after = json.dumps(nb, indent=1, ensure_ascii=False)
                fp.write_text(after, encoding="utf-8")
                message = f"Successfully inserted cell at index {insert_at} in {fp}"
                summary = build_change_summary(fp, before, after)
                if summary:
                    return ToolResult(message, {"change_summary": summary})
                return message
            ...
            after = json.dumps(nb, indent=1, ensure_ascii=False)
            fp.write_text(after, encoding="utf-8")
            message = f"Successfully edited cell {cell_index} in {fp}"
            summary = build_change_summary(fp, before, after)
            if summary:
                return ToolResult(message, {"change_summary": summary})
            return message
