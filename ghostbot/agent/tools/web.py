...
from ghostbot.utils.helpers import build_image_content_blocks

if TYPE_CHECKING:
    from ghostbot.config.schema import WebSearchConfig
...
def _validate_url_safe(url: str) -> tuple[bool, str]:
    ...
    from ghostbot.security.network import validate_url_target
        ...
        try:
            async with httpx.AsyncClient(proxy=self.proxy, follow_redirects=True, max_redirects=MAX_REDIRECTS, timeout=15.0) as client:
                async with client.stream("GET", url, headers={"User-Agent": USER_AGENT}) as r:
                    from ghostbot.security.network import validate_resolved_url
