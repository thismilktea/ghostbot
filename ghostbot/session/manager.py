...
from ghostbot.config.paths import get_legacy_sessions_dir
...
class Session:
    ...
    def touch_file(self, path: str, max_files: int = 15) -> None:
        ...
        # Keep top max_files to prevent infinite expansion
...
class SessionManager:
    ...
    def _get_legacy_session_path(self, key: str) -> Path:
        """Legacy global session path (~/.ghostbot/sessions/)."""
        safe_key = safe_filename(key.replace(":", "_"))
    ...
    def _normalize_active_files(self, active_files: dict[str, float] | None) -> dict[str, float]:
        ...
        for path, touched_at in (active_files or {}).items():
            key = _normalize_active_file_path(path, self.workspace)
            if not key:
                continue
            ...
            with open(path, encoding="utf-8") as f:
                for line in f:
                    ...
                    if data.get("_type") == "metadata":
                        ...
                        active_files = self._normalize_active_files(data.get("active_files", {}))
                    else:
    ...
    def save(self, session: Session) -> None:
        ...
        normalized_active_files = self._normalize_active_files(session.active_files)
        session.active_files = normalized_active_files
